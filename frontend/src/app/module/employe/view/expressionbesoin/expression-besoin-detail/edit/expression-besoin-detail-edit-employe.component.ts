...
@Component({
  selector: 'app-expression-besoin-detail-edit-employe',
  templateUrl: './expression-besoin-detail-edit-employe.component.html'
})
export class ExpressionBesoinDetailEditEmployeComponent extends AbstractEditController<ExpressionBesoinDetailDto, ExpressionBesoinDetailCriteria, ExpressionBesoinDetailEmployeService>   implements OnInit {
   ...
   public async openCreateExpressionBesoin(expressionBesoin: string) {
        const isPermistted = await this.roleService.isPermitted('ExpressionBesoin', 'edit');
        if (isPermistted) {
             this.expressionBesoin = new ExpressionBesoinDto();
